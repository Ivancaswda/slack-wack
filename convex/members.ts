...
export const remove  = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (member.role === 'admin') {
            throw  new Error('admin canot be remoevd')
        }

        const isSelf = currentMember._id === args.id

        if (isSelf && currentMember.role === 'admin') {
            throw new Error('you`re trynna to remove yourself')
        }
        ...
        const [messages, reactions, conversations] = await Promise.all([
            ...
            ctx.db.query('conversations').filter((q) => q.or(q.eq(q.field('memberOneId'), member._id),
