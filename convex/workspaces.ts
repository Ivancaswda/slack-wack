...
const generateCode = () => {
    const chars = '0123456789abcdefjhsoykgffjgjbcxxz';
    const code = Array.from(
        { length: 6 },
        () => chars[Math.floor(Math.random() * chars.length)] // правильное ограничение
    ).join('');
    return code;
}
...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const member = await ctx.db.query('members').withIndex('by_workspace_id_user_id',
            (q) => q.eq('workspaceId', args.id).eq('userId', userId)).unique()
...
export const join = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existingMember) {
            throw  new Error('Already a member of this workspaca')
        }
